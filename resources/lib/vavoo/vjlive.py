...
def get_stalker_channels(genres=False):
	...
	from .stalker import StalkerPortal, get_genres, new_mac
...
def getchannels(type=None, group=None):
	...
	if getSetting("vavoo") == "true" and not type == "stalker":
		from .vavoo_tv import get_vav_channels
...
def livePlay(name, type=None, group=None):
	...
	o = ListItem(name)
	log("Spiele %s" % url)
	hls_stream = "hls" in url or "m3u8" in url
	ffmpegdirect_available = bool(getCondV("System.HasAddon(inputstream.ffmpegdirect)"))
	if hls_stream:
		inputstream = "inputstream.ffmpegdirect" if ffmpegdirect_available and getSetting("hlsinputstream") == "0" else "inputstream.adaptive"
	else:
		inputstream = "inputstream.ffmpegdirect" if ffmpegdirect_available else ""
	if inputstream:
...
def makem3u():
	...
	for name in getchannels():
		m3u.append('#EXTINF:-1 group-title="Standard",%s\n%s\n' % (name.strip(), action_url("livePlay", name=name.strip())))
...
def channels(items=None, type=None, group=None):
	...
	for name in results:
		...
		if not name in lines:
			cm.append(("zu TV Favoriten hinzufügen", "RunPlugin(%s)" % action_url("addTvFavorit", name=name)))
			plot = ""
		else:
			plot = "[COLOR gold]TV Favorit[/COLOR]"
			cm.append(("von TV Favoriten entfernen", "RunPlugin(%s)" % action_url("delTvFavorit", name=name)))
		cm.append(("Einstellungen", "RunPlugin(%s)" % action_url("settings")))
		cm.append(("m3u erstellen", "RunPlugin(%s)" % action_url("makem3u")))
