...
def group_tv(params):
	if params.get("type")=="vavoo":
		from .vavoo_tv import vavoo_groups
...
def search(params):
	...
	if not history or params.get("newsearch"):
		...
		for a in history:
			cm = [("Suchverlauf löschen", "RunPlugin(%s)" % action_url("delete_search", id=params["id"])), ("Suche löschen", "RunPlugin(%s)" % action_url("delete_search", id=params["id"], single=a))]
