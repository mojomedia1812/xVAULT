...
cachepath = os.path.join(addonprofile, "vavoo-cache")
...
SETTING_PREFIX = "vavoo."
VAVOO_SETTING_IDS = set([
	"auto",
	"count",
	"stream_count",
	"hlsinputstream",
...
def _setting_id(setting):
	if setting.startswith(SETTING_PREFIX):
		return setting
	if setting in VAVOO_SETTING_IDS:
		return SETTING_PREFIX + setting
	return setting

def getSetting(setting):
	return addon.getSetting(_setting_id(setting))

def setSetting(setting, value):
	return addon.setSetting(_setting_id(setting), value)

def action_name(action):
	if not action or action.startswith(ACTION_PREFIX):
		return action
	return ACTION_PREFIX + action

def action_url(action, **params):
	params["action"] = action_name(action)
	return "%s?%s" % (sys.argv[0], convertPluginParams(params))

def route_params(params):
	params = dict(params)
	if not params.get("action") and params.get("name"):
		params["action"] = "livePlay"
	if params.get("action"):
		params["action"] = action_name(params["action"])
	return params
...
def log(msg, header=""):
	...
	if header: header+="\n"
	out = "\n####VAVOOTO####\n%s%s\n########" % (header, msg)
	mode = xbmc.LOGINFO if getSetting("debug") == "true" else xbmc.LOGDEBUG
	xbmc.log(out, mode)
...
def addDir(name, params, iconimage="DefaultFolder.png", isFolder=True, context=None):
	...
	plot = " "
	if not context: context.append(("Einstellungen", "RunPlugin(%s)" % action_url("settings")))
	if name == "TV Favoriten (Live)":
		plot = "[COLOR gold]Liste der eigenen Live Favoriten[/COLOR]"
		context.append(("Alle Favoriten entfernen", "RunPlugin(%s)" % action_url("delallTvFavorit")))
...
def getIcon(name):
	vavoo_art = os.path.join(os.path.dirname(__file__), "art", "%s.png" % name)
	if os.path.exists(vavoo_art): return vavoo_art
...
def url_for(params):
	return "%s?%s" % (sys.argv[0], convertPluginParams(route_params(params)))
