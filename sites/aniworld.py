...
class source:
            ...
            for title in titles:
                ...
                for href, series_title in links:
                    if _titles_match(search_variants, series_title):
                        ranked_matches.append((self._series_match_score(search_variants, href, series_title), href, series_title))
                if ranked_matches:
                    ranked_matches.sort(key=lambda item: item[0], reverse=True)
                    score, href, series_title = ranked_matches[0]
                    matches.append({'source': href})
                    if log_utils:
                        logger.info('AniWorld - Match: %s | title: %s | score: %s' % (href, series_title, score))
                if matches:
                    break
    ...
    def _series_match_score(self, search_variants, href, series_title):
    ...
    def resolve(self, url):
        try:
            ...
            try:
                ...
                if html_redirect:
                    if log_utils:
                        logger.info('AniWorld - Resolved via HTML redirect: %s' % html_redirect[:80])
