...
create table if not exists public.xvault_installations (
  ...
  addon_version text,
  kodi_version text,
  os_class text not null default 'unknown',
  device_class text not null default 'unknown',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create table if not exists public.xvault_sessions (
  session_id_hash text primary key,
  install_id_hash text not null references public.xvault_installations(install_id_hash) on delete cascade,
...
create table if not exists public.xvault_events (
  id bigserial primary key,
  install_id_hash text not null references public.xvault_installations(install_id_hash) on delete cascade,
  session_id_hash text references public.xvault_sessions(session_id_hash) on delete set null,
  event_name text not null,
  event_group text not null default 'lifecycle',
...
create index if not exists idx_xvault_events_occurred_at on public.xvault_events(occurred_at desc);
create index if not exists idx_xvault_events_event_name on public.xvault_events(event_name, occurred_at desc);
create index if not exists idx_xvault_sessions_install_last_seen on public.xvault_sessions(install_id_hash, last_seen desc);
create index if not exists idx_xvault_installations_last_seen on public.xvault_installations(last_seen desc);

alter table public.xvault_installations enable row level security;
alter table public.xvault_sessions enable row level security;
alter table public.xvault_events enable row level security;

revoke all on table public.xvault_installations from public, anon, authenticated;
revoke all on table public.xvault_sessions from public, anon, authenticated;
revoke all on table public.xvault_events from public, anon, authenticated;
revoke all on sequence public.xvault_events_id_seq from public, anon, authenticated;
...
begin
  ...
  if event_name not in ('installation_created', 'app_start', 'app_stop', 'heartbeat') then
...
create or replace view public.xvault_daily_overview as
select
  ...
  count(distinct install_id_hash) as unique_installations
from public.xvault_events
group by 1, 2, 3, 4;

create or replace view public.xvault_online_overview as
select
  count(*) filter (where is_online) as currently_online,
  count(*) as total_installations,
  max(last_seen) as last_seen
from public.xvault_installations;

revoke all on table public.xvault_daily_overview from public, anon, authenticated;
revoke all on table public.xvault_online_overview from public, anon, authenticated;
